...
$(function() {
  ...
  function maybeDrawScatter(
      OPTION1, OPTION2, geoDictX, geoDictY, geoDictPop, locations) {
    if (locationSelected() && selectedTwo()) {
  ...
  function selectedThird(evt) {
    return selectedTwo() && evt.target !== CURR_CHECKED['one'] &&
  ...
  function createOption(dropdownOptions, text, value) {
    ...
    opt.value = value;
    if (text.includes('geoId')) {  // remove geoId suffix
  ...
  $('#place-types-states').change(function() {
    ...
    $.get('data/placeObject.json', function(nestedLocations) {
      ...
      if (stateTypeElem.value.length > 0 &&
          stateTypeElem.value !== CURR_COUNTRY) {
        let countyTypeElemkeys =
            Object.keys(nestedLocations[CURR_COUNTRY][stateTypeElem.value])
                .sort();
  ...
  let allCategories = [
    'Demographics', 'Health', 'Economics', 'Business', 'Crime', 'Education',
    'Employment', 'Housing'
  ];
  ...
  let exploreMenu = {
    initialize: function(json, urlParams) {
      ...
      let traverse = (node, parentElement) => {
        let showIfSelectedOption = (argString) => {
          if (selectedOptionList.includes(argString)) {
            ...
            document.getElementById(`checkbox-${argString}`)
        ...
        if (node.type.toLowerCase() === 'property') {
          ...
          if (node.children.length > 0) {
            ...
            htmlString +=
                `<a id="${node.argString}" class="expand-link" data-argstring="${
                    node.argString}">
      ...
      let toggleChildren = (evt) => {
        ...
        evt.currentTarget.querySelector('.right-caret')
        ...
        if (evt.target.classList.contains('checkbox')) {
          ...
          if (selectedThird(evt)) {
            ...
            if (toggleVal) {
              ...
              updateOptions();
              if (locationSelected()) {  // clear chart if <2 variables
                drawScatter(OPTION1, OPTION2, {}, {}, {}, []);
              }
            }
          }
  ...
  function sendRequest(
      reqUrl, isZip, isGet = true, data = {}, dataCallback = function() {},
  ...
  function getLocationsAndPlot(geoDictX, geoDictY, geoDictPop) {
    ...
    $('#enter-threshold').click(function() {
      ...
      if (Number.isNaN(minInput)) {
        ...
        MIN_POP = minInput;
        MAX_POP = maxInput;
        maybeDrawScatter(OPTION1, OPTION2, geoDictX, geoDictY, geoDictPop,
                         locations);
      }
    });
  }
  ...
  function getPayloadArray(dataCallback, errorCallback) {
    ...
    let xAxis, yAxis, popCount = undefined;

    // x-axis
    sendRequest(
        '/bulk/place-obs', true, false,
        getRequestData(LOCATION_IN, popX, pvX), function(payload) {
          xAxis = payload;
    ...
    sendRequest(
        '/bulk/place-obs', true, false,
        getRequestData(LOCATION_IN, popY, pvY), function(payload) {
          yAxis = payload;
          dataCallback(xAxis, yAxis, popCount);
        }, errorCallback);
    ...
    sendRequest(
        '/bulk/place-obs', true, false,
        getRequestData(LOCATION_IN, 'Person', []), function(payload) {
          popCount = payload;
          dataCallback(xAxis, yAxis, popCount);
        }, errorCallback);
  }
  ...
  function requestDataAndPlot() {
    ...
    getPayloadArray(
      function(xAxis, yAxis, popCount) {
        ...
        if (xAxis && Object.keys(xAxis).length > 0 && yAxis &&
        ...
        } else {  // if payloadArray is empty, do not plot
  ...
  function drawScatter(
    ...
    if (locations.length > 0) {
      ...
      for (let locationId of locations) {
        ...
        if ((typeof geoDictX[locationId] !== 'undefined') &&
            (typeof geoDictY[locationId] !== 'undefined')) {
          // make sure values are present
          if ((typeof geoDictX[locationId]['value'] !== 'undefined') &&
            ...
            if (p > MIN_POP && p < MAX_POP) {
              if (PER_CAPITA) {
                downloadArr.push([location, getPerCapita(x, p, OPTION1),
              ...
              } else {  // neither swap nor per capita,
                // datapoint = [x,y,name]
    ...
    function drawChart(xleg, yleg, hObj, vObj) {
      ...
      setTimeout(function() {
        ...
        let options = {
          ...
          hAxis: hObj,
          vAxis: vObj,
          legend: {position: 'right', maxLines: 7, textStyle: {fontSize: 12}},
          chartArea: {'width': '40%'},
          dataOpacity: 0.6,
        };
