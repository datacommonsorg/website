 ...
 * @param {string} options.logoUrl - URL template for logo
 */
export function renderDomainInfo(domainInfo, options = {}) {
  ...
  // Update description title if present
  const descTitleElement = document.querySelector('#descriptionTitle');
  if (descTitleElement && domainInfo.descriptionTitle) {
    descTitleElement.textContent = domainInfo.descriptionTitle;
  }

  // Update description body if present
  const descBodyElement = document.querySelector('#descriptionBody');
  if (descBodyElement && domainInfo.descriptionBody) {
    descBodyElement.textContent = domainInfo.descriptionBody;
  }

  // Update logo
  const logoElement = document.querySelector(logoSelector);
  if (logoElement && domainInfo.logoPresent === "true" && logoUrl) {
    // Replace logo element content but preserve domain name
    const domainNameText = domainInfo.domainName || 'Health Organization';
    logoElement.innerHTML = `<img id="domainLogoImg" class="domain-logo-image" src="${logoUrl}" alt="${domainNameText} logo" /><span id="domainName" class="logo-text">${domainNameText}</span>`;
...
// Set global functions for backward compatibility with non-module scripts
if (typeof window !== 'undefined') {
  window.getApiRoot = getApiRoot;
  window.fetchData = fetchData;
  window.renderDomainInfo = renderDomainInfo;
}
