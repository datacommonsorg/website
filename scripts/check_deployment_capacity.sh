...
RESULT=$(jq -n \
  ...
  | [ $workloads[] | select(.kind != "DaemonSet") | . as $w | range(.replicas) | { identifier: "\($w.kind)/\($w.name) (Instance \(.) )", nodepool: $w.nodepool, cpu: $w.cpu, mem: $w.mem } ]
...
if echo "$RESULT" | jq -e '.deployable' > /dev/null; then
    ...
    echo "$RESULT" | jq -r '.failures[] | "  - \(.identifier) -> Requires: \(.cpu)m CPU, \(.mem)Mi Memory"'
    exit 1
else
    ...
    echo "$RESULT" | jq -r '.failures[] | "  - \(.identifier) -> Requires: \(.cpu)m CPU, \(.mem)Mi Memory (Limiting factor: \(.reason))"'
    echo ""
