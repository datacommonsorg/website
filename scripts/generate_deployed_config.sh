...
# 2. If not found by project, check if it matches an env filename directly
if [[ -z "$ENV_YAML" ]]; then
...
# 3. If still not found, list available envs/projects and exit
if [[ -z "$ENV_YAML" ]]; then
...
MIXER_RENDER=$(helm template dc-mixer "$ROOT/mixer/deploy/helm_charts/mixer" \
  --namespace "$NAMESPACE" \
  -f "$ENV_YAML" \
