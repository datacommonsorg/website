...
./run_local_docker.sh [--env_file|-e <env.list file path>] [--mode|m run] [--schema_update|s] 
  [container|-c all|data] [--release|r latest|stable]

./run_local ./run_local_docker.sh [--env_file|-e <env.list file path>] --mode|m build_run 
  --image <custom image name:tag> [container|-c all|service] [--release|-r latest|stable]
...
--container|-c all|service|data
  ...
  * data: Only run the data container. Only use this if you are running in a hybrid mode, 
    ...
    Only valid with 'run' and 'build_run' modes.
...
--package <target package name:tag>
  ...
  Only valid with 'build_upload' and "upload" modes.
...
run_data() {
  ...
  # Hybrid setup: local input dir, output to Cloud SQL
...
run_service() {
  # Custom-built image
  if [ -n "$IMAGE" ]; then
    # Hybrid setup: data in Cloud SQL
...
# Commons containers). Needed for hybrid setups and docker tag/push.
check_app_credentials() {
...
# This is to get credentials to authenticate Docker to GCP. Needed for docker
# tag
get_docker_credentials() {
...
help_text="Please try again. See '--help' for correct usage.\n"
...
# Incompatible options that are ambiguous (most are silently ignored)
# Reset options to valid values
#---------------------------------------------------------------------------
if [ "$MODE" == "build_run" ] && [ "$CONTAINER" == "data" ]; then
  echo -e "${RED} Error:${NC} Invalid mode and container combination."
  echo -e $help_text
  exit 1
fi
...
if [ "$SCHEMA_UPDATE" == true ]; then
  if [ "$MODE" != "run" ]; then
    echo -e "${RED} Error:${NC} Invalid mode and schema update combination."
    echo -e $help_text
    exit 1
  fi
  if [ "$CONTAINER" == "service" ]; then
    echo -e "${RED} Error:${NC} Invalid container and schema update combination."
    echo -e $help_text
    exit 1
  fi
fi

# Service-only hybrid mode
if [[ "$INPUT_DIR" == *"gs://"* ]] && [[ "$OUTPUT_DIR" == *"gs://"* ]]; then
  if [ "$CONTAINER" != "service" ]; then
    echo -e "${YELLOW} Warning:${NC} Invalid container option '$CONTAINER' for local service-only mode."
    echo -e "Ignoring and running service container only.\n"
    RUN="service"
    sleep 3
  fi
  if [ "$MODE" != "run" ]; then
    echo -e "${YELLOW} Warning:$NC Invalid mode '$MODE'."
    echo -e "Ignoring and running service container only.\n"
    MODE="run"
    sleep 3
  fi
...
# Data-only hybrid mode
if [[ "$INPUT_DIR" != *"gs://"* ]] && [[ "$OUTPUT_DIR" == *"gs://"* ]]; then
  if [ "$CONTAINER" != "data" ]; then
    echo -e "${YELLOW} Warning:${NC} Invalid container option '$CONTAINER' for local data-only mode."
