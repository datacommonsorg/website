...
for project_id in $LOG_SOURCE_PROJECT_IDS; do
    ...
    # Check if sink already exists and skip if so.
    if gcloud logging sinks list --filter="name=${SINK_NAME}" --project="${project_id}" --format="value(name)" | grep -q "."; then
        echo "Sink ${SINK_NAME} already exists in project ${project_id}. Skipping..."
        continue
    fi
