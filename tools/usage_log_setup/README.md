...
Before running the script, follow these instructions to set up the [BigQuery command line tool](https://docs.cloud.google.com/bigquery/docs/quickstarts/load-data-bq) and the [`gcloud` command line tool](https://docs.cloud.google.com/sdk/docs/install).
...
Your logs will be stored in a BigQuery table of your choosing that can be in either the same GCP project as your logs or a different one. In the second case, you'll configure a log router to route from your first project into a separate `DESTINATION_PROJECT` for the project where you plan to set up BigQuery.
...
This script generates a BigQuery dataset in the GCP project where you plan to store your logs. Then, we create a log sink that routes `stdout` to this dataset for each GCP project that recieves logs (specify these as a list in `LOG_SOURCE_PROJECT_IDS` in `.env`).
