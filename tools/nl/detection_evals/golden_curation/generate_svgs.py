...
flags.DEFINE_string('input_folder', 'input',
                    'The input directory of with files of queries to curate')
flags.DEFINE_string('output_folder', 'output',
                    'The output directory to save generated embeddings')
flags.DEFINE_string(
    'svg_cache', 'svg_cache.json',
...
def get_relevant_svgs(var_str: str, var_list: List[str],
                      svg_cache: SvgCache) -> Set[str]:
  """
  ...
  for svg in var_svgs:
    found_svgs.update([
        g for g in svg_cache.children_svg.get(svg, [])
  ...
  nl_response = requests.post(
      ...
      json={},
      timeout=None)
  found_svgs.update(nl_response.json().get('debug', {}).get('counters', {}).get(
      'INFO', {}).get('filtered_svs', [[]])[0])
  return found_svgs


def get_svgs_for_var_strs(
    merged_var_str_to_vars: Dict[str, List[str]],
    svg_cache: SvgCache) -> Tuple[Dict[str, List[str]], List[str]]:
  """
  ...
  for var_str, vars in merged_var_str_to_vars.items():
    try:
      ...
      print(e)
      continue
    formatted_svgs, _ = get_hierarchy_format(list(svg_set),
                                             svg_cache.children_svg,
                                             _MAX_SVG_CHILDREN_LEVELS)
    svgs[var_str] = []
...
def get_svgs(query_info: Dict[str, QueryInfo], svg_cache: SvgCache):
  ...
  for q in list(query_info.values()):
    for var_str in q.var_str_to_vars.keys():
      ...
      if curr_svg_results:
        results.extend([
            [q.query_num, q.query_str, var_str, svg] for svg in curr_svg_results
        ])
      else:
