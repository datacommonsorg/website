...
def get_gcs_folder(gcs_folder: str | None, failed_attempts_path: str | None,
  ...
  if failed_attempts_path:
    return os.path.join(GCS_FILE_DIR_RETRIES, date_folder)

  if use_bigquery:
    return os.path.join(GCS_FILE_DIR_FULL, date_folder)

  return os.path.join(GCS_FILE_DIR_NL, date_folder)
...
def export_to_json(sv_metadata_list: list[dict[str, str | list[str]]],
  ...
  local_file_path = os.path.join(EXPORTED_FILE_DIR, filename)
  ...
  os.makedirs(os.path.join(EXPORTED_FILE_DIR, "failures"), exist_ok=True)
