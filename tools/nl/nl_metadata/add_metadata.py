...
def main():
    ...
    for i in range(0, len(embeddings), batch_size):
        ...
        for dcid, sentence in zip(*curr_batch.to_dict("list").values()):
            new_row = {
                "dcid": dcid, 
                "sentence": sentence,
                "constraintProperties": '' # Initialize constraintProperties here since it cannot be natively extracted
            }
            ...
            if "constraintProperties" in dcid_data:
                ...
                for node in dcid_data["constraintProperties"]["nodes"]:
                    ...
                    # Some SVs have both "name" and "dcid", others only have "dcid"
                    if "name" in node:
                    ...
                    if "name" in first_node:
                        curr_constraint_properties.append(f'{constrained_prop}={first_node.get("name")}')
                    else:
                        curr_constraint_properties.append(f'{constrained_prop}={first_node.get("dcid")}')
    ...
    new_embeddings_df.to_csv("tools/nl/nl_metadata/alyssaguo_statvars.csv", index=False)
