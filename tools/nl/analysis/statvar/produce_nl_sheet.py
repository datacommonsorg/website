...
if 'Name' not in df_14k_dcids.columns:
    ...
    df_14k_dcids = pd.DataFrame.from_dict({'dcid': df_14k_dcids_list, "Name": names_14k})
...
df_joined = df_14k_dcids.merge(df_1300_curated, how="left", on=["dcid"]).fillna("")
# Remove the duplicated columns.
df_joined['Name_x'] = np.where(df_joined["Name_x"] != "", df_joined["Name_x"], df_joined['Name_y'])
...
df_joined['Description'] = np.where(df_joined["Chart Title"] != "", df_joined["Chart Title"], df_joined['Description'])

pd.set_option('display.max_columns', None)
print(df_joined.head(10))
print(df_joined.shape)

cols_to_keep = ["dcid","Name","Description","Override_Alternatives","Curated_Alternatives"]
df_joined = df_joined[cols_to_keep]

# No need to include SVs were there is no Name or Description or Curated_Alternative.
df_joined.drop(df_joined[(df_joined["Name"] == "") & (df_joined["Description"] == "") & (df_joined["Curated_Alternatives"] == "")].index, inplace = True)
