...
def login(driver):
  ...
  try:
    ...
    print(
        "IAP login page not detected within timeout. Assuming already logged in or not an IAP protected page."
    )
  ...
  try:
    ...
    secret_path = secret_client.secret_version_path(_SECRET_PROJECT,
                                                    _SECRET_NAME, 'latest')
    ...
    print(
        f"Login failed: A required element was not found or clickable within timeout: {e}"
    )
...
)  # Overcomes limited resource problems in some environments
chrome_options.add_argument(
    "--window-size=1920,1080")  # Set a consistent window size for snapshots
...
try:
  ...
  if environment == "staging":
    ...
    if driver.current_url != initial_url_for_login and not driver.current_url.startswith(
        base_url):
      print(
          f"Redirected after login. Navigating back to: {initial_url_for_login}"
      )
      driver.get(initial_url_for_login)
  else:
  ...
  for name, url in urls_to_snapshot.items():
    ...
    try:
      ...
      time.sleep(
          5
      )  # Fallback sleep if explicit wait fails to ensure some loading time
