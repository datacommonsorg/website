...
}> = ({ chartConfigCategory, placeDcid, varToTopics }) => {
  ...
  chartConfigCategory.blocks.forEach((block) => {
    block.columns.forEach((column) => {
      column.tiles.forEach((tile) => {
        if (!_.isEmpty(tile.statVarKey) && !_.isEmpty(varToTopics[tile.statVarKey[0]])) {
          for (const topic of varToTopics[tile.statVarKey[0]]) {
            ...
            const targetMatches = topic.dcid.match(/dc\/topic\/sdg_(\d\d?\.\w\w?)/);
