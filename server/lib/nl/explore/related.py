...
def compute_related_things(
    ...
    had_place_fallback = bool(
        fallback
        and fallback.newPlace
        and fallback.origPlace
        and fallback.newStr != fallback.origStr
    )
...
def _get_sdg_ancestors_recursive(topics: List[str], result: List[str], added: Set[str]):
...
def prune_related_topics(related_things, uttr):
    ...
    all_topics = list(
        set(
            (
                [x["dcid"] for x in related_things["parentTopics"]]
    ...
    valid_topics, _ = utils.sv_existence_for_places(
        [x.dcid for x in uttr.places], all_topics, uttr.counters
    )
...
def generate_follow_up_questions(query: str, related_topics: List[str]) -> List[str]:
