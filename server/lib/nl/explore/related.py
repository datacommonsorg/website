...
from server.lib.nl.explore.utils import _GEMINI_MODEL
from server.lib.nl.explore.utils import _RELATED_QUESTIONS_QUERY
...
class FollowUpQuestions(BaseModel):
  """The follow up questions generated for the `Keep Exploring` component.

  Attributes:
    questions: A list of strings containing each question generated by Gemini corresponding to one related topic.
  """
  questions: List[str]
...
def followup_with_geminipro(query: str, related_topics: List[str]) -> List[str]:
  """ Makes the call to Gemini Pro to generate the follow up questions for
      the Keep Exploring component based on the initial query and related topics.

      Args:
      query: The initial query made by the user.
      related_topics: The parent,peer, and child topics identified by detect.
  ...
  for i in range(_RETRIES):
    try:
      gemini_response = gemini.models.generate_content(
          model=_GEMINI_MODEL,
          contents=_RELATED_QUESTIONS_QUERY.format(
      ...
      logging.error(f'[explore_follow_up]: Iteration {i+1} - {e}',
                    exc_info=True)
      continue
