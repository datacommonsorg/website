  ...
  """
    ...
    for stat_var_key, spec in category.stat_var_spec.items():
        sv_exist = _sv_places_exist(places, spec.stat_var, stat_vars_existence,
                                    place_dcid)
        if spec.denom:
            denom_exist = _sv_places_exist(places, spec.denom,
                ...
                for t in column.tiles:
                    ...
                    if t.type == subject_page_pb2.Tile.TileType.BAR:
                        if not t.comparison_places:
                            filtered_keys = [
                                k for k in t.stat_var_key if place_exist_keys[k]
                            ]
                        else:
                            # Should be the comparison places.
                            comparison_exist_keys = _exist_keys_category(
                                t.comparison_places, category,
                                stat_vars_existence, place_dcid)
                            filtered_keys = [
                                k for k in t.stat_var_key
                                if comparison_exist_keys[k]
                            ]
                        ...
                        filtered_keys = [
                            k for k in t.stat_var_key if child_exist_keys[k]
                        ]
                    else:
                        new_tiles.append(t)
                        continue
