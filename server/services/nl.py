...
def _highlight_svs(sv_df):
  return sv_df[sv_df['Cosine Score'] > 0.4]['SV'].values.tolist()

def _filtered_svs_list(sv_df):
  sv_df = sv_df.drop(sv_df[sv_df['Cosine Score'] < 0.3].index)
...
def _bucket_to_name(key, fixed_props):
  parts = key.split(',')
...
# the missing V.
def _get_buckets(defn, fixed_prefixes):
  parts = defn.split(',')
  fixed_parts = []
  cpv_parts = []
  for part in parts:
...
def _chart_config(place_dcid, main_place_type, main_place_name, child_places_type, 
                  highlight_svs, sv2name, fixed_props, peer_buckets):
  #@title
  chart_config = {}
  ...
  for sv in highlight_svs:
    ...
    for key, svs in peer_buckets.items():
      ...
      tiles.append({
            'title': _bucket_to_name(key, fixed_props),
            'type': 'LINE',
  ...
  for key, svs in peer_buckets.items():
    ...
    blocks.append({
        'title': _bucket_to_name(key, fixed_props) + ' in ' + main_place_name,
        'columns': [{
            'tiles': [tile]
        }]
    })
...
class Model:
  ...
  def _related_svgs(self, svs_list, relevant_places):
    ...
    svgs_info = self._get_svg_info(relevant_places, list(svgs))['data']
    return svgs_info


  def _related_places(self, dcid):
    # Get related places using Place API
    related_places = self._get_related_places(dcid)
    related_places['nearbyPlaces'] += [dcid]
    related_places['similarPlaces'] += [dcid]

    return related_places

  def _peer_buckets(self, sv2definition, svs_list, fixed_prefixes):
    peer_buckets = {}
    for sv, defn in sv2definition.items():
      keys = _get_buckets(defn, fixed_prefixes)
  ...
  def search(self, query, tmp_place_dcid):
    ...
    related_places = self._related_places(place_dcid)
    ...
    # TODO(jehangiramjad): implement this. For now, using all of query.

    # Step 3: Find matching SVs with scores.
    ...
    highlight_svs = _highlight_svs(result)
    relevant_svs = _filtered_svs_list(result)

    # Step 5: get related SVGs and all info.
    svgs_info = self._related_svgs(relevant_svs, all_relevant_places)
    ...
    peer_buckets = self._peer_buckets(sv2definition, relevant_svs, FIXED_PREFIXES)

    # Step 8: Produce Chart Config JSON.
    # Returning the chart config JSON.
    # TODO(jehangiramjad): in debug mode, return other helpful information as well.
    return _chart_config(place_dcid, main_place_type, main_place_name,
                          child_places_type, highlight_svs, sv2name, 
                          FIXED_PROPS, peer_buckets)
