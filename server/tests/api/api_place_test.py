...
class TestApiPlaceI18nName(unittest.TestCase):
  ...
  def test_parent_places(self, mock_fetch_data, mock_names):
    ...
    response = app.test_client().get('/api/place/name/i18n?dcid=geoId/05&hl=ru')
    assert response.status_code == 200
...
class TestApiGetStatVarsUnion(unittest.TestCase):
  ...
  def test_api_get_stat_vars_union(self, send_request):
    ...
    send_request.side_effect = side_effect
    response = app.test_client().post('/api/place/stat-vars/union', json=req)
    assert response.status_code == 200
...
class TestApiRankingChart(unittest.TestCase):
  ...
  def test_api_ranking_chart_not_earth(
    ...
    response = app.test_client().get('/api/place/ranking_chart/' + test_dcid)
    assert response.status_code == 200
    ...
    expected_data = {
        sv1: {
            ...
            'numDataPoints': 2,
            'exploreUrl': "/ranking/sv1/place_type/parent_dcid1?h=test_dcid",
            'sources': [source1]
        },
        sv2: {
            'date':
