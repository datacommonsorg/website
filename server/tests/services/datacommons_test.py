...
class TestServiceDataCommonsV2Node(unittest.TestCase):
  ...
  TEST_DATA_DIR = os.path.join(_dir, "..", "test_data", "datacommons")
  ...
  def test_termination_condition_max_pages_fetched(self, mock_post):
    response_with_next_token = self.get_json('v2node_response_with_next_token')
    response_without_next_token = self.get_json(
        'v2node_response_without_next_token')
  ...
  def test_merging_paged_responses(self, mock_post):
    ...
    self.assertEqual(
        v2node(['dc/1', 'dc/2'], '->{property1,property2}', max_pages=3),
        self.get_json('v2node_expected_merged_response'))

  @mock.patch('server.services.datacommons.post')
  def test_merging_property_responses(self, mock_post):
    properties_with_next_token = self.get_json(
        'v2node_properties_with_next_token')
    properties_without_next_token = self.get_json(
    ...
    self.assertEqual(v2node(['dc/1', 'dc/2'], '->', max_pages=3),
                     self.get_json('v2node_expected_merged_properties'))
