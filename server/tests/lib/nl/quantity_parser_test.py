...
class TestQuantityParser(unittest.TestCase):
  ...
  @parameterized.expand([
      ...
      ('cities which have 1M people', '((EQ 1000000.0) idx:12)'),
      ('cities with as much as 1M people', '((EQ 1000000.0) idx:11)'),
      ('cities where the median age is somewhere near 50', None),
      ('cities where population is over 1M', '((GT 1000000.0) idx:26)'),
      ('cities where population is above 1M', '((GT 1000000.0) idx:26)'),
      ('cities where population exceeds 1M', '((GT 1000000.0) idx:23)'),
      ('cities with population more than 1M', '((GT 1000000.0) idx:22)'),
      ('cities with population greater than 1M', '((GT 1000000.0) idx:22)'),
      ('cities with population higher than 1M', '((GT 1000000.0) idx:22)'),
      ('cities with population upwards of 1M', '((GT 1000000.0) idx:22)'),
      ('cities where population is under 100K', '((LT 100000.0) idx:26)'),
      ('cities where population is below 100K', '((LT 100000.0) idx:26)'),
      ('cities with population less than 100K', '((LT 100000.0) idx:22)'),
      ('cities with population lesser than 100K', '((LT 100000.0) idx:22)'),
      ('cities with population lower than 100K', '((LT 100000.0) idx:22)'),
      ('cities with population greater than or equal to 1B',
       '((GE 1000000000.0) idx:22)'),
      ('cities with population greater than or equals 1B',
       '((GE 1000000000.0) idx:22)'),
      ('population from 1B people', '((GE 1000000000.0) idx:10)'),
      ...
      ('countries where gdp growth rate was greater than 2.5%',
       '((GT 2.5) idx:35)'),
      ('counties with 10 million dollar homes', ''),
      ('counties where home prices exceed 10 million dollars',
       '((GT 10000000.0) idx:26)'),
      ...
      ('countries with over 1 Trillion dollars GDP',
       '((GT 1000000000000.0) idx:14)'),
      ('countries with under 1 billion euro GDP', '((LT 1000000000.0) idx:14)'),
      ('countries with over 10B people', '((GT 10000000000.0) idx:14)'),
