...
def filter_chart_configs_for_category(
    ...
    place_type: str) -> List[ServerChartConfiguration]:
  """
  ...
  if place_type == "Continent":
    # Do not respect the is_overview filter for the continent overview page.
    return copy.deepcopy(chart_config)
  ...
  for server_chart_config in chart_config:
    server_chart_config.blocks = [
        block for block in server_chart_config.blocks
        if block.is_overview or place_type == 'Continent'
    ]
