...
def _append_fallback_response(query, response, path_finder,
  ...
  gemini_model = GEMINI_FLASH if is_feature_enabled(
      ENABLE_GEMINI_2_5_FLASH) else GEMINI_FLASH_LITE
  token_limit = GEMINI_FLASH_TOKEN_LIMIT if gemini_model == GEMINI_FLASH else GEMINI_FLASH_LITE_TOKEN_LIMIT
  ...
  gemini_response = gemini_client.models.generate_content(
      model=gemini_model, contents=fallback_prompt)
...
def _fulfill_traversal_query(query):
  ...
  gemini_model = GEMINI_FLASH if is_feature_enabled(
      ENABLE_GEMINI_2_5_FLASH) else GEMINI_FLASH_LITE
  token_limit = GEMINI_FLASH_TOKEN_LIMIT if gemini_model == GEMINI_FLASH else GEMINI_FLASH_LITE_TOKEN_LIMIT
  path_finder = PathFinder(query, gemini_client, gemini_model_str=gemini_model)
  ...
  try:
    ...
    input_tokens = gemini_client.models.count_tokens(
        contents=final_prompt, model=gemini_model).total_tokens
